...
export default function StorePill({ name, short = false }) {
  ...
  return (
    <span
      ...
      style={{ backgroundColor: color.bg, color: color.text }}
    >
      <span className="md:hidden w-5 h-5 flex items-center justify-center">{name[0]}</span>
      <span className="hidden md:inline px-1.5 py-0.5">{name}</span>
    </span>
  );
}
